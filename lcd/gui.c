...
{  
	...
	num=num-' ';//?õ?t?????
	LCD_SetWindows(x,y,x+size/2-1,y+size-1);//???õ??????????????
	if(!mode) //???????
	{
	...
	}else//??????
	{
		for(pos=0;pos<size;pos++)
		{
		    ...
		    {   
				POINT_COLOR=fc;              
		        if(temp&0x01)LCD_DrawPoint(x+t,y+pos);//??h????    
		        temp>>=1; 
		    }
		}
	}
	  ...
	  { 	LCD_SetWindows(x,y,x+16-1,y+16-1);
		    for(i=0;i<16*2;i++)
		    {
		    	...
		    	{	
					...
					{
						POINT_COLOR=fc;
						if(tfont16[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//??h????
						x++;
						if((x-x0)==16)
						{
							x=x0;
			...
			for (k=0;k<HZnum;k++) 
			{
			  ...
			  { 	LCD_SetWindows(x,y,x+24-1,y+24-1);
				    for(i=0;i<24*3;i++)
				    {
							...
							{
								POINT_COLOR=fc;
								if(tfont24[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//??h????
								x++;
								if((x-x0)==24)
								{
									x=x0;
			  ...
			  { 	LCD_SetWindows(x,y,x+32-1,y+32-1);
				    for(i=0;i<32*4;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							...
							{
								POINT_COLOR=fc;
								if(tfont32[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//??h????
								x++;
								if((x-x0)==32)
								{
									x=x0;
...
void Display_ModuleStatus(unsigned char Light_Num, unsigned char Status)      //???????????????????????
{
   ...
   switch(Light_Num)
     {
     case 1: LCD_SetWindows(X1_pos,sy,width_x+X1_pos,ey);//???????????
             break;
     case 2: LCD_SetWindows(X2_pos,sy,width_x+X2_pos,ey);//???????????
             break;
     case 3: LCD_SetWindows(X3_pos,sy,width_x+X3_pos,ey);//???????????
             break;
     case 4: LCD_SetWindows(X4_pos,sy,width_x+X4_pos,ey);//???????????
             break;
     case 5: LCD_SetWindows(X5_pos,sy,width_x+X5_pos,ey);//???????????
             break;
     case 6: LCD_SetWindows(X6_pos,sy,width_x+X6_pos,ey);//???????????
             break;   
     case 7: LCD_SetWindows(X7_pos,sy,width_x+X7_pos,ey);//???????????
             break; 
     case 8: LCD_SetWindows(X8_pos,sy,width_x+X8_pos,ey);//???????????
             break;   
     case 9: LCD_SetWindows(X9_pos,sy,width_x+X9_pos,ey);//???????????
             break;  		              
     case 10: LCD_SetWindows(X10_pos,sy,width_x+X10_pos,ey);//???????????
             break;  
     case 11: LCD_SetWindows(X11_pos,sy,width_x+X11_pos,ey);//???????????
             break;  
     case 12: LCD_SetWindows(X12_pos,sy,width_x+X12_pos,ey);//???????????
             break;  
     case 13: LCD_SetWindows(X13_pos,sy,width_x+X13_pos,ey);//???????????
             break; 
     case 14: LCD_SetWindows(X14_pos,sy,width_x+X14_pos,ey);//???????????
             break;   
     case 15: LCD_SetWindows(X15_pos,sy,width_x+X15_pos,ey);//???????????
             break;  						 
   ...
   switch(Status)
    {
     case 0: color = GRAY;    //????????û??????g??
             
             break;      
     case 1: color = WHITE;   //????????g?????
            
             break;    
     case 2: color = GREEN;   //????????g??????????
...
void Display_Area_RedGlint(unsigned char const bmp[], unsigned char Area, unsigned char original)      //???28?????l?????
{
  ...
  unsigned char  Start_X1 = 244,  End_X1 = 251, 		
	               Start_X2 = 236,  End_X2 = 243,  
								 Start_X3 = 228,  End_X3 = 235, 
	               Start_X4 = 220,  End_X4 = 227,
								 Start_X5 = 212,  End_X5 = 219,
                 Start_X6 = 204,  End_X6 = 211,  
								 Start_X7 = 196,  End_X7 = 203,
                 Start_X8 = 188,  End_X8 = 195,
								 Start_X9 = 180,  End_X9 = 187,
                 Start_X10 = 172, End_X10 = 179, 
								 Start_X11 = 164, End_X11 = 171,
	               Start_X12 = 156, End_X12 = 163,
								 Start_X13 = 148, End_X13 = 155,
	               Start_X14 = 140, End_X14 = 147, 
								 Start_X15 = 132, End_X15 = 139,
	               Start_X16 = 124, End_X16 = 131,
								 Start_X17 = 116,  End_X17 = 123,
	               Start_X18 = 108,  End_X18 = 115, 
								 Start_X19 = 100,  End_X19 = 107,
                 Start_X20 = 92,  End_X20 = 99,
								 Start_X21 = 84,  End_X21 = 91,
                 Start_X22 = 76,  End_X22 = 83,  
								 Start_X23 = 68,  End_X23 = 75,
								 Start_X24 = 60,  End_X24 = 67,
								 Start_X25 = 52,  End_X25 = 59,
								 Start_X26 = 44,  End_X26 = 51,  
								 Start_X27 = 36,  End_X27 = 43,
								 Start_X28 = 28,  End_X28 = 35;     //28?????X????????????
   ...
   switch(Area)
   {
   case 1:  LCD_SetWindows(Start_X1,Start_Y,End_X1,End_Y);//???????????
            break;
   case 2:  LCD_SetWindows(Start_X2,Start_Y,End_X2,End_Y);//???????????
            break;
   case 3:  LCD_SetWindows(Start_X3,Start_Y,End_X3,End_Y);//???????????
            break;
   case 4:  LCD_SetWindows(Start_X4,Start_Y,End_X4,End_Y);//???????????
            break;      
   case 5:  LCD_SetWindows(Start_X5,Start_Y,End_X5,End_Y);//???????????
            break;
   case 6:  LCD_SetWindows(Start_X6,Start_Y,End_X6,End_Y);//???????????
            break;
   case 7:  LCD_SetWindows(Start_X7,Start_Y,End_X7,End_Y);//???????????
            break;
   case 8:  LCD_SetWindows(Start_X8,Start_Y,End_X8,End_Y);//???????????
            break;
   case 9:  LCD_SetWindows(Start_X9,Start_Y,End_X9,End_Y);//???????????
            break;
   case 10: LCD_SetWindows(Start_X10,Start_Y,End_X10,End_Y);//???????????
            break;       
   case 11: LCD_SetWindows(Start_X11,Start_Y,End_X11,End_Y);//???????????
            break;
   case 12: LCD_SetWindows(Start_X12,Start_Y,End_X12,End_Y);//???????????
            break;
   case 13: LCD_SetWindows(Start_X13,Start_Y,End_X13,End_Y);//???????????
            break;
   case 14: LCD_SetWindows(Start_X14,Start_Y,End_X14,End_Y);//???????????
            break;
   case 15: LCD_SetWindows(Start_X15,Start_Y,End_X15,End_Y);//???????????
            break;
   case 16: LCD_SetWindows(Start_X16,Start_Y,End_X16,End_Y);//???????????
            break;      
   case 17: LCD_SetWindows(Start_X17,Start_Y,End_X17,End_Y);//???????????
            break;
   case 18: LCD_SetWindows(Start_X18,Start_Y,End_X18,End_Y);//???????????
            break;
   case 19: LCD_SetWindows(Start_X19,Start_Y,End_X19,End_Y);//???????????
            break;
   case 20: LCD_SetWindows(Start_X20,Start_Y,End_X20,End_Y);//???????????
            break;
   case 21: LCD_SetWindows(Start_X21,Start_Y,End_X21,End_Y);//???????????
            break;
   case 22: LCD_SetWindows(Start_X22,Start_Y,End_X22,End_Y);//???????????
            break;        
   case 23: LCD_SetWindows(Start_X23,Start_Y,End_X23,End_Y);//???????????
            break;
   case 24: LCD_SetWindows(Start_X24,Start_Y,End_X24,End_Y);//???????????
            break;
   case 25: LCD_SetWindows(Start_X25,Start_Y,End_X25,End_Y);//???????????
            break;
   case 26: LCD_SetWindows(Start_X26,Start_Y,End_X26,End_Y);//???????????
            break;
   case 27: LCD_SetWindows(Start_X27,Start_Y,End_X27,End_Y);//???????????
            break;
   case 28: LCD_SetWindows(Start_X28,Start_Y,End_X28,End_Y);//???????????
            break;       
   default: __NOP(); 
            break;
   }
...
{     	    
			...
			u16 x1,y1, x2, color;
			x1=275,x2=295;	
      m_max_value=VALUE[max_value];//????õ?U??????????
