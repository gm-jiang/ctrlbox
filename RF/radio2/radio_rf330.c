...
void RF330_Config(void)
{
    ...
    /* Config interrupt */
    CMT2300A_ConfigInterrupt_RF330(CMT2300A_INT_SEL_TX_DONE, /* Config INT1 */
                                   CMT2300A_INT_SEL_PKT_OK   /* Config INT2 */
    );
#endif

    /* Enable interrupt */
    CMT2300A_EnableInterrupt_RF330(CMT2300A_MASK_TX_DONE_EN | CMT2300A_MASK_PKT_DONE_EN);

    /* Disable low frequency OSC calibration */
    CMT2300A_EnableLfosc_RF330(FALSE);

    /* Use a single 64-byte  FIFO for either Tx or Rx */
    CMT2300A_EnableFifoMerge_RF330(TRUE); //�ϲ�FIFO

    CMT2300A_SetFifoThreshold_RF330(FIFO_TH); // ������ֵ
