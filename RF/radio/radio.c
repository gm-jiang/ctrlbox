...
//    CMT2300A_EnableTxDin(TRUE);//使能GPIO引脚为发射	
//		CMT2300A_ConfigTxDin(CMT2300A_TX_DIN_SEL_GPIO1);//选择GPIO1作为波形发射口
//	  CMT2300A_EnableTxDinInvert(FALSE);//发射波形使用默认状态不做取反。

//    /* Disable low frequency OSC calibration */
//    CMT2300A_EnableLfosc(FALSE);
//    
//    /* Use a single 64-byte FIFO for either Tx or Rx */
//    CMT2300A_EnableFifoMerge(TRUE); //合并FIFO
//    
//    CMT2300A_SetFifoThreshold(FIFO_TH); // 设置阈值
//    
...
///* Disable low frequency OSC calibration */
  ...
  CMT2300A_EnableLfosc(FALSE);
	CMT2300A_ConfigGpio(CMT2300A_GPIO3_SEL_DOUT);//配置GPIO123的功能	
	CMT2300A_EnableTxDin(TRUE);//配置GPIO为输出使能
	CMT2300A_ConfigTxDin(CMT2300A_TX_DIN_SEL_GPIO3);//选择GPIO1为输出角位
